    ...
    partial struct Job_Grid_Realloc : IJobParallelFor
    {
        ...
        public void Execute(int _Index)
        {
            ...
            else
            {
                ...
                float _RatioShrinkGrowth = 0.7f;
                float _Progress = _Realloc._Timer / _Realloc._Duration;

                if (_Progress <= _RatioShrinkGrowth) // Shrink
                    _LocalTransform.Scale = math.lerp(_Realloc._Scale, 0, _Progress / _RatioShrinkGrowth);
                else // Growth
                {
                    _LocalTransform.Position = _Realloc._Position;
                    _LocalTransform.Scale = math.lerp(0, _Realloc._Scale, (_Progress - _RatioShrinkGrowth) / (1 - _RatioShrinkGrowth));
                }
